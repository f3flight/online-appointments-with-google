...
class testHTTPServer_RequestHandler(BaseHTTPRequestHandler):
    ...
    cal = gcal.AppointmentManager('credentials.json', cfg)
    ...
    def do_POST(self):
        ...
        if postvars.get('slot'):
            try:
                if not self.cal.create_appointment(postvars['slot'], postvars):
                    failed = True
            except:
                failed = True
                if postvars['email']:
...
def run():
    ...
    settings = {'address': '127.0.0.1',
                ...
                'keyfile': None,
                'certfile': None}
    if os.path.exists('http_config.yaml'):
        with open('http_config.yaml', 'r') as f:
